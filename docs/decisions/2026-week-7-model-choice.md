...
## Context
- The Week 6 logistic regression baseline caught fewer than one in five truly critical (ESI 1) patients, which Dr. Reyes flagged as unacceptable for a triage-support tool.
- The ED Board and Martina Griffith (Clinical IT) asked for a defensible recommendation on whether a more complex model justifies its extra training and inference cost, not simply "the best model."
...
## Reasoning
- Training time is a one-off cost; inference time is paid on every patient indefinitely. Gradient Boosting wins decisively on the cost that actually recurs (0.015 ms/pred) while tying the tuned Random Forest on the metric that matters clinically (ESI-1 recall).
...
- Aggregate accuracy and macro-F1 are lower for Gradient Boosting than for the baseline or tuned Random Forest, but neither of those metrics reflects Dr. Reyes's stated priority; optimising for them over ESI-1 recall would repeat the exact trap Week 6 exposed.
